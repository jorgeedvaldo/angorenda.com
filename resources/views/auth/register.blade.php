...
<head>
    ...
    <title>Criar Conta - Angorenda</title>
...
<body class="font-sans antialiased bg-gray-50 text-gray-900 min-h-screen flex flex-col justify-center py-12 sm:px-6 lg:px-8">
    ...
    <div class="sm:mx-auto sm:w-full sm:max-w-md">
        ...
        <p class="mt-2 text-center text-sm text-gray-600">
            Ou <a href="/admin/login" class="font-medium text-brand-600 hover:text-brand-500">entrar na conta existente</a>
        </p>
    </div>
    ...
    <div class="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
        <div class="bg-white py-8 px-4 shadow-sm border border-gray-100 sm:rounded-2xl sm:px-10">
            ...
            @if ($errors->any())
                <div class="bg-red-50 border-l-4 border-red-500 p-4 mb-6 rounded-md">
                    <div class="flex">
                        ...
                        <div class="ml-3">
                            <h3 class="text-sm font-medium text-red-800">Foram encontrados erros:</h3>
            ...
            <form class="space-y-5" action="/signup" method="POST">
                ...
                <div>
                    <label for="name" class="block text-sm font-medium text-gray-700">Nome ou Empresa</label>
                    <div class="mt-1">
                        <input id="name" name="name" type="text" value="{{ old('name') }}" required class="appearance-none block w-full px-3 py-2.5 border border-gray-300 rounded-xl shadow-sm placeholder-gray-400 focus:outline-none focus:ring-brand-500 focus:border-brand-500 sm:text-sm">
                    </div>
                </div>
                ...
                <div>
                    <label for="phone" class="block text-sm font-medium text-gray-700">Telemóvel / WhatsApp (opcional)</label>
                    <div class="mt-1">
                        <input id="phone" name="phone" type="text" value="{{ old('phone') }}" class="appearance-none block w-full px-3 py-2.5 border border-gray-300 rounded-xl shadow-sm placeholder-gray-400 focus:outline-none focus:ring-brand-500 focus:border-brand-500 sm:text-sm">
                    </div>
                </div>

                <div>
                    <label for="type" class="block text-sm font-medium text-gray-700">Tipo de anunciante</label>
                    <div class="mt-1">
                        <select id="type" name="type" required class="appearance-none block w-full px-3 py-2.5 border border-gray-300 rounded-xl shadow-sm focus:outline-none focus:ring-brand-500 focus:border-brand-500 sm:text-sm bg-white">
                            <option value="individual" {{ old('type') == 'individual' ? 'selected' : '' }}>Individual / Particular</option>
                            <option value="empresa" {{ old('type') == 'empresa' ? 'selected' : '' }}>Agência / Empresa</option>
                        </select>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="mt-1">
                        <input id="password_confirmation" name="password_confirmation" type="password" required class="appearance-none block w-full px-3 py-2.5 border border-gray-300 rounded-xl shadow-sm placeholder-gray-400 focus:outline-none focus:ring-brand-500 focus:border-brand-500 sm:text-sm">
                    </div>
                </div>
                ...
                <div>
                    <button type="submit" class="w-full flex justify-center py-3 px-4 border border-transparent rounded-xl shadow-sm text-sm font-medium text-white bg-brand-600 hover:bg-brand-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-brand-500 transition-colors">
                        Criar conta e Anunciar Imóveis
                    </button>
                </div>
            </form>
